...
class Articles_model extends CI_Model
{
	...
	{
		$query_str = "SELECT articles.id,topics.name, `title`,`content`, `Upvotes`,`Downvotes`,user_student.username FROM `articles` JOIN user_student ON articles.user_id=user_student.id 
	...
	{
		$auth_query = $this->db->get_where('user_student',array('username'=>$this->session->userdata('unnamed')));
	...
	{
		$query_str = "SELECT user_student.username FROM `articles` JOIN user_student ON articles.user_id=user_student.id WHERE articles.id='$art_id'";
	...
	public function topic_list()
	{
 	...
 	public function get_comments($art_id)
 	{
 		...
 		$query_str = "SELECT comment,user_id,Upvotes,Downvotes,datetime,article_id,id FROM comments_articles WHERE `article_id`= '$art_id'";
		$query = $this->db->query($query_str);
		return $query->result_array(); 
